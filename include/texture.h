...
public:
    stripe_texture() { }
    stripe_texture(texture *t0, texture *t1): even(t0), odd(t1) { }
    virtual vec3 value(float u, float v, const vec3& p) const {
        float sines = sin(80*p.y());
        if (sines < 0)
            return odd->value(u, v, p);
        else
            return even->value(u, v, p);
...
public:
    test_texture() { }
    test_texture(texture *t0, texture *t1): even(t0), odd(t1) { }
    virtual vec3 value(float u, float v, const vec3& p) const {
        float sines = sin(20*p.x())*(1-cos(20*p.y()));
        if (sines < 0)
            return odd->value(u, v, p);
        else
            return even->value(u, v, p);
